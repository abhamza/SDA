...
PortableGrayMap* createImageFromFile(const char* filename)
{
  ...
  if (file == NULL)
  {
    fprintf(stderr, "createImageFromFile - fopen failed !\n");
    return NULL;
  }
  ...
  {
    fprintf(stderr, "createImageFromFile - magic number (1)\n");
    fclose(file);
    return NULL;
  }
  ...
  {
    fprintf(stderr, "createImageFromFile - magic number (2)\n");
    fclose(file);
    return NULL;
  }
  ...
  {
    fprintf(stderr, "createImageFromFile - width\n");
    fclose(file);
    return NULL;
  }
  ...
  {
    fprintf(stderr, "createImageFromFile - height\n");
    fclose(file);
    return NULL;
  }
  ...
  {
    fprintf(stderr, "createImageFromFile - max value\n");
    fclose(file);
    return NULL;
  }
  ...
  if (res == NULL)
  {
    fprintf(stderr, "createImageFromFile - malloc failed !\n");
    fclose(file);
    return NULL;
  }
  res->type = type;
  ...
  for (size_t i = 0; i < res->height; ++i)
    for (size_t j = 0; j < res->width; ++j)
    {
      ...
      if (value < 0)
      {
        fprintf(stderr, "createImageFromFile - " \
                        "fscanf failed (line: %lu, column: %lu) !\n", i, j);
        deleteImage(res);
        fclose(file);
        return NULL;
      }
...
int saveImageToFile(const PortableGrayMap* image, const char* filename)
{
  ...
  if (file == NULL)
  {
    fprintf(stderr, "saveImageToFile - fopen failed !");
    return -1;
  }
